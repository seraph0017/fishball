...
app.include_router(media.media_group_router)
...
def index_handle(request: Request):
    return RedirectResponse(url=app.url_path_for("media_group_index", media_group_id=1))
...
def login(data: UserLogin):
    ...
    token = lm.create_access_token(
        data=dict(sub=user.email), expires=datetime.timedelta(minutes=60)
    )
